...
Open `dist/win-x64/desktop/Airlift.exe`, or install `dist/installer/Airlift-Setup-0.2.8.exe`. Both are built locally and include the .NET runtime. Builds use ordinary folders, not bundled executables: keep the DLLs and runtime files beside each EXE. The installer packages the complete desktop and CLI folders.
